                    ...
                    <div className="flex flex-col w-fit max-w-full h-fit">
                        {
                            recenltyPlayed.items.map((tracks,index) => (
                                <div key={index} className="flex rounded py-2 bg-black hover:bg-indigo-800">
                                    ...
                                    <div key={index}  className="flex flex-col justify-center text-white ml-5">
                                        <div key={index}>{tracks.track.name}</div>
                                        <div index={index} className="flex justify-center">
                                        {tracks.track.artists.map((artist, i) => (
                                            <React.Fragment key={artist?.id}>
                                            <NavLink to={`http://localhost:5173/artist/${artist?.id}`}>
                                                {artist.name}
                                            </NavLink>
                                            {i < tracks.track.artists.length - 1 && ', '}
                                            </React.Fragment>
                                        ))}
                                        </div>
                                    </div>
